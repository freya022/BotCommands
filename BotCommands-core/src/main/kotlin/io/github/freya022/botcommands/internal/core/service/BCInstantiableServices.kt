...
internal class BCInstantiableServices internal constructor(serviceProviders: ServiceProviders, serviceContainer: BCServiceContainerImpl) : InstantiableServices {
    internal val availableProviders: Set<ServiceProvider> = serviceProviders.allProviders.mapNotNullTo(sortedSetOf()) { provider ->
        ...
        if (serviceError.errorType == ServiceError.ErrorType.UNAVAILABLE_PARAMETER) {
            if (serviceError.nestedError?.errorType == ServiceError.ErrorType.NON_UNIQUE_PROVIDERS) {
        ...
        if (provider.isLazy) {
            when (serviceError.errorType) {
                ServiceError.ErrorType.UNKNOWN, ServiceError.ErrorType.NO_USABLE_PROVIDER, ServiceError.ErrorType.PROVIDER_RETURNED_NULL, ServiceError.ErrorType.NO_PROVIDER, ServiceError.ErrorType.NON_UNIQUE_PROVIDERS -> throwInternal(
