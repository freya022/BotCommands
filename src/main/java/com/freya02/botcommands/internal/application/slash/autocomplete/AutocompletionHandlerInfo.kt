...
class AutocompletionHandlerInfo(
    ...
    init {
        ...
        parameters = MethodParameters.of<SlashParameterResolver>(method, mapOf()) { kParameter, paramName, _ -> //TODO pass builders
...
//                    .stream()
//                    .filter { p: SlashCommandParameter -> p.name == parameter.applicationOptionData.effectiveName }
//                    .findAny()
...
//        }
    ...
    fun checkParameters(info: SlashCommandInfo) {
        ...
        autocompleteParameterLoop@ for (autocompleteParameter in parameters) {
            ...
            if (autocompleteParameter.methodParameterType != MethodParameterType.COMMAND) continue

            autocompleteParameter as AutocompleteCommandParameter

            for (slashCommandParameter in slashOptions) {
                if (slashCommandParameter.methodParameterType != MethodParameterType.COMMAND) continue

                slashCommandParameter as SlashCommandParameter

                if (slashCommandParameter.name == autocompleteParameter.name) {
                    checkParameter(slashCommandParameter, autocompleteParameter)
                    continue@autocompleteParameterLoop
                }
            }

            throwUser("Couldn't find parameter named '${autocompleteParameter.name}'")
        }
    }
    ...
    private fun checkParameter(
        ...
        val slashParameterType = slashCommandParameter.type
        val autocompleteParameterType = autocompleteParameter.type
